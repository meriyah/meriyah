 ...
 * @param type Binding type
 */
export function addNameToExports(parser: ParserState, key: string): void {
  if (parser.exportedNames !== undefined && key !== '') {
    if (parser.exportedNames['#' + key]) {
      report(parser, Errors.DuplicateExportBinding, key);
    }
    parser.exportedNames['#' + key] = ScopeMasks.Undeclared;
  }
}
 ...
 * @param type Binding type
 */
export function addBindingToExports(parser: ParserState, key: any): void {
  if (parser.exportedBindings !== undefined && key !== '') {
    let hashed = '#' + key;
    parser.exportedBindings[hashed] = ScopeMasks.Undeclared;
  }
}
