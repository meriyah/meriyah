...
describe('Lexer - Regular expressions', () => {
  const tokens: ([Context, string, string, string] | [Context, string, string, string, Partial<NormalizedOptions>])[] =
    [
  ...
  for (const [ctx, op, value, flags, options] of tokens) {
    it(`scans '${op}' at the end`, () => {
      const parser = new Parser(op, { validateRegularExpressionWithRuntime: true, ...options });
  ...
  function fail(name: string, source: string, context: Context) {
    it(name, () => {
      const parser = new Parser(source, { validateRegularExpressionWithRuntime: true });
