...
describe('src/lexer/scan', () => {
  describe('#scanSingleToken()', () => {
    ...
    it('should recognise comments', () => {
      const state = create('// this is a comment');
      const token = scanSingleToken(state, Context.None);
      t.deepEqual(token, Token.EOF);
    });
    it('should recognise arithmetic operators', () => {
      ...
      token = scanSingleToken(state, Context.None);
    ...
    it('should throw on unterminated string literals', () => {
      const state = create("'abdc");
      t.throws(() => scanSingleToken(state, Context.None));
    });
    // tslint:enable quotemark
    it('should recognise keywords', () => {
      const state = create('let const');
      let token = scanSingleToken(state, Context.None);
    ...
    it('should recognise identifiers', () => {
      const state = create('test _test a1');
      let token = scanSingleToken(state, Context.None);
      t.deepEqual(token, Token.Identifier);
