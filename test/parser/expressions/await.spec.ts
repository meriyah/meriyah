...
describe('Expressions - Await', () => {
  ...
  for (const arg of [
    ...
    it(`'use strict'; function f() { ${arg}) }`, () => {
      expect(() => {
        parseSource(`'use strict'; function f() { ${arg}) }`, undefined, Context.None).toThrow();
      });
    });

    it(`let f = () => {${arg})`, () => {
      expect(() => {
        parseSource(`let f = () => {${arg})`, undefined, Context.None).toThrow();
      });
    });

    it(`let f = () => {${arg})`, () => {
      expect(() => {
        parseSource(`let f = () => {${arg})`, undefined, Context.OptionsNext).toThrow();
      });
    });
